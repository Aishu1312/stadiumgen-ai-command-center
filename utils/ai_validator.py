...
def validate_api_key(api_key: Optional[str]) -> bool:
    ...
    if not api_key or not str(api_key).strip() or str(api_key).strip() == "your_gemini_api_key_here":
...
def validate_model(model_name: str, supported_models: List[str]) -> bool:
    ...
    expected_name = model_name if model_name.startswith("models/") else f"models/{model_name}"
    if expected_name not in supported_models:
